...
from stimuli import RangeResponseSlider

# Initialize the response slider
response_slider = RangeResponseSlider(
    win,
    position=(0, 0),  # Centered
    length=15,  # Length in degrees
    height=0.5,  # Height in degrees
    color=[.5, .5, .5],
    borderColor=[-1, -1, -1],
    borderWidth=5.,
    range=(0.55, 55),  # Slider range
    marker_position=10,
    markerColor=[0.25, 0.25, 0.25],
    width_proportion=0.1,  # Fixed typo
    slider_type='natural',
)
